	...
	class TGM_Plugin_Activation {
			...
			if ( ! empty( $message ) ) {
				...
				if ( current_user_can( 'install_plugins' ) ) {
					if ( $install_link_count > 0 ) {
						$action_links['install'] = sprintf(
							$link_template,
							translate_nooped_plural( $this->strings['install_link'], $install_link_count, '{%= text_domain %}' ),
					...
					if ( $update_link_count > 0 ) {
						$action_links['update'] = sprintf(
							$link_template,
							translate_nooped_plural( $this->strings['update_link'], $update_link_count, '{%= text_domain %}' ),
							esc_url( $this->get_tgmpa_status_url( 'update' ) )
						);
					}
				}
				...
				if ( current_user_can( 'activate_plugins' ) && $activate_link_count > 0 ) {
					$action_links['activate'] = sprintf(
						$link_template,
						translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, '{%= text_domain %}' ),
		...
		public function show_tgmpa_version() {
			echo '<p style="float: right; padding: 0em 1.5em 0.5em 0;"><strong><small>',
				esc_html( sprintf( _x( 'TGMPA v%s', '%s = version number', '{%= text_domain %}' ), self::TGMPA_VERSION ) ),
				'</small></strong></p>';
		}
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function get_plugin_status_text( $slug ) {
			...
			return sprintf(
				_x( '%1$s, %2$s', '%1$s = install status, %2$s = update status', '{%= text_domain %}' ),
				$install_status,
				$update_status
			);
		}
		...
		public function no_items() {
			printf( wp_kses_post( __( 'No plugins to install, update or activate. <a href="%1$s">Return to the Dashboard</a>', '{%= text_domain %}' ) ), esc_url( self_admin_url() ) );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		}
		...
		protected function get_row_actions( $item ) {
			...
			if ( ! $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
				$actions['install'] = _x( 'Install %2$s', '%2$s = plugin name in screen reader markup', '{%= text_domain %}' );
			} else {
				// Display the 'Update' action link if an update is available and WP complies with plugin minimum.
				if ( false !== $this->tgmpa->does_plugin_have_update( $item['slug'] ) && $this->tgmpa->can_plugin_update( $item['slug'] ) ) {
					$actions['update'] = _x( 'Update %2$s', '%2$s = plugin name in screen reader markup', '{%= text_domain %}' );
				}

				// Display the 'Activate' action link, but only if the plugin meets the minimum version.
				if ( $this->tgmpa->can_plugin_activate( $item['slug'] ) ) {
					$actions['activate'] = _x( 'Activate %2$s', '%2$s = plugin name in screen reader markup', '{%= text_domain %}' );
				}
			}
