	 ...
	 */
		...
		public static function custom_styles( $css = '', $scope = '' ) {
			...
			// Requirements check
				...
				$css = trim( (string) apply_filters( 'wmhook_{%= prefix_hook %}_custom_styles', $css, $scope ) );
				...
				if (
					...
					if ( $output_cached = trim( (string) apply_filters( 'wmhook_{%= prefix_hook %}_library_custom_styles_output', $output_cached, $scope ) ) ) {
			...
			// Processing
				...
				// Setting up replacements array
					...
					if ( ! empty( $theme_options ) && $set_replacements ) {
						...
						$replacements = (array) apply_filters( 'wmhook_{%= prefix_hook %}_library_custom_styles_replacements', $replacements, $theme_options, $css, $scope );
				...
				// Prepare output
					...
					if (
						...
						set_transient( '{%= theme_slug %}_custom_css_debug', apply_filters( 'wmhook_{%= prefix_hook %}_library_custom_styles_output_cache_debug', $output, $scope ) );
						set_transient( '{%= theme_slug %}_custom_css', apply_filters( 'wmhook_{%= prefix_hook %}_library_custom_styles_output_cache', $output, $scope ) );
			...
			// Output
				...
				if ( $output = trim( (string) apply_filters( 'wmhook_{%= prefix_hook %}_library_custom_styles_output', $output, $scope ) ) ) {
					return $output;
				}
